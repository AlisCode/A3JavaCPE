...
public class CheckersGameGUIBoard extends JPanel implements PropertyChangeListener {
    ...
    private void createDamier() {
        ...
        for (int x = 0; x < this.size; x++) {
            for (int y = 0; y < this.size; y++) {
                JPanel case_to_add;
                if ((x + y) % 2 == 1) {
                    case_to_add = new SquareGUI(this.checkersGameControler.getColorBlackSquare());
                } else {
                    case_to_add = new SquareGUI(this.checkersGameControler.getColorWhiteSquare());
                }
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (evt.getPropertyName() == "selectedPieceGUI") {
            this.selectedPieceGUI = (JPanel) evt.getNewValue();
        }
    }
}
